          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <h4 className="text-sm font-bold font-mono text-foreground">
                {selectedModel || modelConfig.model_name || "No Model Selected"}
              </h4>
            </div>
          </div>
          <Badge variant="default" className="text-[10px] font-mono gap-1">
            {isCloud ? <Cloud className="w-3 h-3" /> : <Cpu className="w-3 h-3" />}
            {modelConfig.mode.toUpperCase()}
          </Badge>
        </div>

        <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-primary/15 text-xs font-mono">
          <div className="flex items-center gap-1 text-[11px] text-muted-foreground truncate">
            <Globe className="w-3 h-3 shrink-0" />
            <span className="truncate">{modelConfig.endpoint}</span>
          </div>
          ...
          {activeModelDetails && activeModelDetails.size > 0 && (
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            {isCloud ? <Cloud className="w-4 h-4 text-primary" /> : <Cpu className="w-4 h-4 text-primary" />}
            <h4 className="text-xs font-semibold text-foreground">
              Discovered Provider Models ({localModels?.length || 0})
            </h4>
          </div>
        ...
        {loadingModels ? (
          <div className="py-12 text-center text-xs text-muted-foreground flex flex-col items-center justify-center gap-2">
            ...
            Scanning {modelConfig.provider} models...
          </div>
        ) : isError ? (
          <div className="p-4 rounded-xl bg-destructive/10 border border-destructive/20 text-xs text-destructive space-y-1">
            <p className="font-semibold">Provider Endpoint Unreachable</p>
            <p className="text-muted-foreground">
              Check server connection at {modelConfig.endpoint} or configure credentials in Settings.
            </p>
          </div>
        ) : !localModels || localModels.length === 0 ? (
          <div className="py-12 text-center text-xs text-muted-foreground space-y-2">
            <p>No models returned by active provider.</p>
            {isCloud ? (
              <p className="text-[11px] text-muted-foreground">Specify your cloud model name in Settings.</p>
            ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground pt-1.5 border-t border-border/40 font-mono">
                    <HardDrive className="w-3 h-3" />
                    <span>{model.size > 0 ? formatBytes(model.size) : "Cloud Hosted"}</span>
                  </div>
                </motion.button>
              );
            })}
